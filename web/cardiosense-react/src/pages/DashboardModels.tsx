...
export default function DashboardModels() {
      ...
      >
        <div className="text-center">
          ...
          <p className="text-muted">
            Loading model metrics from ML pipeline...
          </p>
        </div>
      </div>
    );
      ...
      >
        <div className="text-center">
          ...
          <p className="text-muted mt-2">
            Could not load model metrics. Please refresh.
          </p>
        </div>
      </div>
    );
  ...
  const bestModel = metrics.models.reduce(
    (best, m) => (m.metrics.accuracy > best.metrics.accuracy ? m : best),
    metrics.models[0],
  );
  ...
  > = {
    XGBoost: {
      ...
      bg: "#ffebee",
      icon: "bi-tree",
      desc: "Gradient boosted trees with Optuna hyperparameter tuning",
    },
    LightGBM: {
      color: "#108a7e",
      bg: "#e0f7f5",
      icon: "bi-lightning",
      desc: "Leaf-wise tree growth for faster training",
    },
    "Random Forest": {
      color: "#ef8a43",
      bg: "#fff3e0",
      icon: "bi-forest",
      desc: "Bagging ensemble with 446 estimators",
    },
    "Soft Voting": {
      color: "#7c4dff",
      bg: "#ede7f6",
      icon: "bi-collection",
      desc: "Probability averaging across calibrated base models",
    },
    Stacking: {
      color: "#00bcd4",
      bg: "#e0f7fa",
      icon: "bi-stack",
      desc: "Meta-learner on top of base model predictions",
    },
  };
  ...
  return (
    <>
      <div className="dashboard-header" data-reveal>
        ...
        <p className="text-muted">
          Complete pipeline details: 5 calibrated models with threshold
          optimization.
        </p>
      </div>
      ...
      <div className="dashboard-stats-grid">
        {[
          ...
          },
          {
            ...
            value: formatNumber(metrics.threshold ?? 0, 4),
            color: "var(--amber)",
          },
          {
            icon: "bi-check-circle",
            label: "Calibration",
            value: metrics.config?.calibrationMethod ?? "sigmoid",
            color: "var(--primary)",
          },
        ].map((stat, idx) => (
          <div
          ...
          >
            <div className="d-flex justify-content-between align-items-start">
              ...
              <div
                className="stat-icon"
                style={{ opacity: 0.15, fontSize: "1.8rem", color: stat.color }}
              >
        ...
        {metrics.models.map((model, idx) => {
          ...
          return (
            <div key={idx} className="col-md-6 col-lg-4" data-reveal>
              <div
                className="dashboard-panel h-100 position-relative"
                style={{
      ...
      <div className="dashboard-panel" data-reveal>
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h3 className="m-0">
            <i className="bi bi-table text-muted"></i> Detailed Model Comparison
          </h3>
          <span
            className="badge"
      ...
      <div className="dashboard-panel mt-4" data-reveal>
        <h3 className="mb-4">
          <i className="bi bi-gear text-muted"></i> Optimal Hyperparameters
        </h3>
        <div className="row g-4">
          {[
            ...
            <div key={idx} className="col-md-4">
              ...
              >
                <h5 style={{ color: model.color, marginBottom: "12px" }}>
                  {model.name}
                </h5>
      ...
      <div className="dashboard-panel mt-4" data-reveal>
        <h3 className="mb-4">
          <i className="bi bi-shield-check text-muted"></i> Pipeline
          Configuration
        </h3>
          ...
          {[
            {
              ...
              desc: "Soft Voting (probability averaging) & Stacking (meta-learner on calibrated probabilities)",
              color: "#7c4dff",
            },
            {
              icon: "bi-check-circle",
              title: "Calibration",
              desc: `Sigmoid calibration applied to all base models. Random state: ${metrics.config?.randomState ?? 42}`,
              color: "var(--teal)",
            },
            {
              icon: "bi-bullseye",
              title: "Objective",
              desc: `Balanced accuracy optimization targeting ${((metrics.config?.targetRecall ?? 0.83) * 100).toFixed(0)}% recall and ${((metrics.config?.targetAccuracy ?? 0.78) * 100).toFixed(0)}% accuracy`,
              color: "var(--primary)",
            },
            {
              icon: "bi-database",
              title: "Dataset",
              desc: "68,000+ BRFSS 2015 samples. 13,722 test samples (20% holdout). 11 clinical & lifestyle features.",
              color: "var(--amber)",
            },
            {
              icon: "bi-shield-shaded",
              title: "Validation",
              desc: "5-fold cross-validation with stratification. Precision floor: 72% enforced.",
              color: "#00bcd4",
            },
            {
              icon: "bi-graph-up-arrow",
              title: "Optimization",
              desc: "Threshold search over 41 values from 0.30 to 0.70 with 0.01 step size.",
              color: "#4caf50",
            },
          ].map((item, idx) => (
            <div key={idx} className="col-md-6 col-lg-4">
              ...
              >
                ...
                <div>
                  <h5 style={{ fontSize: "0.9rem", marginBottom: "4px" }}>
                    {item.title}
                  </h5>
                  <p
                    className="text-muted m-0"
